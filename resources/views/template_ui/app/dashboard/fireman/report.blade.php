...
@section('dashboard_content')
    <div class="h-full">
        ...
        <div class="justify-center flex flex-wrap my-2 overflow-x-auto">
          <button onclick="effectButton(this); getReport('pending')" class="active mb-1 p-2 effect-button px-4 mr-2 text-center text-xs border-[1px] border-yellow-500 bg-yellow-500 text-white rounded-lg">
            Laporan Pending
          </button>
          <button onclick="effectButton(this); getReport('all')" class="mb-1 p-2 effect-button px-4 mr-2 text-center text-xs border-[1px] text-yellow-500 font-bold border-yellow-500 rounded-lg">
            Semua Laporan
          </button>
        ...
        <div class="w-full my-4" id="reports">
...
const detailReport = reportId => {
  window.location.href = `{{route('fireman.detail.report',"")}}/${reportId}`;
}
