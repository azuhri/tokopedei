    ...
    <div class="h-full">
        ...
        <div class="flex bg-slate-50 my-6 rounded-lg border-2 border-l-4 border-s-slate-500 hover:border-s-orange-900">
            <div class="p-4 w-1/3 flex relative justify-center items-center border-r-2">
                <img src="{{ $report->user->avatar() }}" class="" alt="">
                <div class="flex items-center absolute bottom-1">
                    <a href="https://wa.me/{{$report->user->phonenumber}}" target="_blank" class="bg-green-600 shadow font-bold p-1 px-2 text-white rounded">
                        <p class="text-[10px] text-center">Kontak WA</p>
                    </a>
                </div>
            </div>
            <div class="flex flex-col justify-center p-4 w-2/3 p-2">
                <div class="my-1">
                    ...
                    <p class="text-[12px]">{{ $report->user->name }}</p>
                </div>
                <div class="my-1">
                    <p class="font-bold text-sm text-slate-700">Email</p>
                    <p class="text-[12px]">{{ $report->user->email }}</p>
                </div>
                <div class="my-1">
                    <p class="font-bold text-sm text-slate-700">Nomor WA</p>
                    <p class="text-[12px]">{{ $report->user->phonenumber }}</p>
                </div>
                @if ($report->report_status == "pending")
                    <div class="flex items-center mt-2">
                        <button onclick="actionReport('ditolak');" class="w-full mr-1 px-2 py-2 bg-red-500 text-white rounded-md text-xs">
                            Tolak Laporan
                        </button>
                        <button onclick="actionReport('diproses');" class="w-full mr-1 px-2 py-2 bg-blue-500 text-white rounded-md text-xs">
                            Terima Laporan
                        </button>
                    </div>
                @elseif($report->report_status == "diproses")
                    <button onclick="actionReport('selesai');" class="w-full mr-1 px-2 py-2 bg-green-600 text-white rounded-md text-xs">
                        Selesaikan Laporan
                    </button>
                @endif
            </div>
        ...
        const actionReport = (status) => {
            ...
            if(confirmation) {
                $.ajax(`{{ route('fireman.update.report', [$report->id, "/"]) }}/${status}`, {
                    ...
                    data: {},
                    success: function(data) {
                        $.Toast("Pesan", `Berhasil ${status == 'ditolak' ? 'menolak' : 'menerima'} laporan`, "success");
                        setTimeout(() => {
                            location.reload();
                        }, 2000);
                    },
